...
const Audio = ({ data, images, setIsCurrentAudioEnded, setIsSwitchedBack }) => {
  ...
  const timeHandler = (e) => {
    if (!ref || !progressBar || !ref.current || !progressBar.current || isPaused) return;
  ...
  const updateTime = (currTime="") => {
    const audioEL = ref.current;
    const currentTime = currTime ? currTime : isNaN(audioEL.currentTime) ? 0 : audioEL.currentTime;
    const duration = isNaN(audioEL.duration) ? 0 : audioEL.duration;
    const currentMinutes = currentTime / 60 > 9 ? parseInt(currentTime / 60) : `0${parseInt(currentTime / 60)}`;
    const currentSeconds = currentTime % 60 > 9 ? parseInt(currentTime % 60) : `0${parseInt(currentTime % 60)}`;
    const currTimeSorted = `${currentMinutes}:${currentSeconds}`;
    const remainedTimeMinutes = (duration - currentTime) / 60 > 9 ? parseInt((duration - currentTime) / 60) : `0${parseInt((duration - currentTime) / 60)}`;
    const remainedTimeSeconds = (duration - currentTime) % 60 > 9 ? parseInt((duration - currentTime) % 60) : `0${parseInt((duration - currentTime) % 60)}`;
    const remainedTimeSorted = `-${remainedTimeMinutes}:${remainedTimeSeconds}`;
      ...
      <div className="playerWrapper">
        ...
        <div className="controlPanel">
          ...
          <div className="controlButtons">
            ...
            <div className="next">
              <img src={images.svgs.nextButtonIcon} alt="previous" onClick={()=>setIsCurrentAudioEnded(true)} />
            </div>
          </div>
        </div>
      </div>
